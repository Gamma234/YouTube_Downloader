...
def show_url_info_in_See_Information( mgmt, main_mgmt ) :
    ...
    try :
        ...
        url_info = YTDL.get_url_information( url= mgmt['url_strVar'].get(), show_dict=show_dict )
...
def See_Information( url_list, main_mgmt ) :

    window = tk.Toplevel( main_mgmt['main_window'] )
    window.title( 'See Information' ) 
    ...
    ret_mgmt['input_frame'] = tk.Frame( window )
    ret_mgmt['input_frame'].pack( side=tk.TOP, pady=5, fill=tk.X )

    ret_mgmt['options_frame'] = tk.Frame( window, height=50 )
    ret_mgmt['options_frame'].pack( side=tk.TOP, pady=5, fill=tk.X )

    ret_mgmt['display_frame'] = tk.Frame( window )
    ...
    ret_mgmt['url_strVar'] = tk.StringVar( ret_mgmt['input_frame'] )
    ret_mgmt['url_strVar'].set( url_list[0] )

    ret_mgmt['url_optMn'] = tk.OptionMenu( ret_mgmt['input_frame'], ret_mgmt['url_strVar'], *url_list )
    ret_mgmt['url_optMn'].config( width=80 )
    ret_mgmt['url_optMn'].pack( side=tk.LEFT, padx=5 )

    ret_mgmt['url_button'] = tk.Button( ret_mgmt['input_frame'], text='GET',
                                        command=( lambda : show_url_info_in_See_Information( ret_mgmt, main_mgmt ) ) )
